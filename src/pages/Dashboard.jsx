...
const Dashboard = () => {
  const { user } = useAuth()
  const { 
    appointments = [], // ⭐ Valores por defecto
    patients = [], 
    doctors = [], 
    stats = {
      totalAppointments: 0,
  ...
  // Estadísticas calculadas con valores seguros
  const dashboardStats = [
    {
      title: "Citas",
      value: (stats?.totalAppointments || 0).toString(),
      change: `+${stats?.appointmentChange || 0}%`,
      subtitle: `${stats?.todayAppointments || 0} citas hoy`,
      icon: Calendar,
      color: "text-blue-600",
    ...
    },
    {
      ...
      change: "+3%",
      subtitle: `${patients.length || 0} pacientes activos`,
      icon: Users,
    ...
    {
      title: "Doctores",
      value: (doctors.length || 0).toString(),
      change: "+0%",
      subtitle: `${doctors.filter(d => d?.status === 'available' || d?.status === 'online').length || 0} disponibles`,
      icon: Activity,
  ...
  // Citas recientes (últimas 5) con manejo seguro
