...
const Patients = () => {
  const { 
    patients, 
    appointments,
    createPatient, // Esta es la función que viene del contexto
    updatePatient, 
    deletePatient 
  } = useApp()
  ...
  // Se llama 'handleCreateSubmit' para no chocar con 'createPatient' del contexto.
  ...
  const handleEdit = (patient) => {
    ...
    setFormData({
      ...
      birth_date: new Date(patient.birth_date), gender: patient.gender || '', address: patient.address || '',
      emergency_contact: patient.emergency_contact || '', emergency_phone: patient.emergency_phone || '',
      insurance: patient.insurance || '', medical_notes: patient.medical_notes || '',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex space-x-2">
          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            <DialogTrigger asChild>
              <Button className="bg-blue-600 hover:bg-blue-700"><Plus className="w-4 h-4 mr-2" />Nuevo Paciente</Button>
            </DialogTrigger>
                  ...
                  <TabsContent value="basic" className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input id="name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="Juan Pérez" required />
                      </div>
                      <div>
                        <Label htmlFor="email">Email</Label>
                        <Input id="email" type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="juan@email.com" />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="phone">Teléfono *</Label>
                        <Input id="phone" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="+54 11 1234-5678" required />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-auto p-0">
                          <Calendar mode="single" selected={formData.birth_date} onSelect={(date) => setFormData({...formData, birth_date: date})} disabled={(date) => date > new Date()} initialFocus />
                        </PopoverContent>
                      </Popover>
                    </div>
                  </TabsContent>
                  <TabsContent value="medical" className="space-y-4">
                    {/* Campos de info médica */}
                  </TabsContent>
      ...
      {/* El resto del JSX sigue aquí abajo... */}
    </div>
  )
}
